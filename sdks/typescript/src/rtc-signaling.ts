...
export function handleConnect(ctx: SignalingContext, frame: StarfishFrame): void {
  const peerId = frame.header.from;
  if (!peerId) return;

  const channels: string[] = (frame.payload?.channels as string[]) ?? DEFAULT_CHANNELS;
...
export async function handleIce(ctx: SignalingContext, frame: StarfishFrame): Promise<void> {
  ...
  await entry.pc.addIceCandidate((frame.payload as any).candidate);
}
...
export const SIGNALING_HANDLERS: Record<
  string,
