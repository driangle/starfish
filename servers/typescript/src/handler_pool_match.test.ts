...
describe("mutual mode", () => {
  ...
  it("returns pool.claim.pending when only one side claims", () => {
    ...
    const pending = findFrames(c1, "pool", "claim");
    expect(pending).toHaveLength(1);
    expect(pending[0].header.replyTo).toBe("claim1");
  ...
  it("matches when both sides claim each other", () => {
    ...
    hub.handler.dispatch(c2, {
      header: { id: "claim2", resource: "pool", method: "claim", kind: "request" },
...
describe("propose mode", () => {
  ...
  it("matches on pool.accept", () => {
    ...
    hub.handler.dispatch(c2, {
      header: { id: "accept1", resource: "pool", method: "accept", kind: "request" },
  ...
  it("sends pool.claim.rejected on reject", () => {
    ...
    hub.handler.dispatch(c2, {
      header: { id: "reject1", resource: "pool", method: "reject", kind: "request" },
      payload: { pool: "lobby", from: c1.id },
    });
...
describe("delegated mode", () => {
  ...
  it("pool.assign fires matches and sends pool.assigned", () => {
    ...
    hub.handler.dispatch(matchmaker, {
      header: { id: "assign1", resource: "pool", method: "assign", kind: "request" },
    ...
    const assigned = findFrames(matchmaker, "pool", "assign");
    expect(assigned).toHaveLength(1);
    expect(assigned[0].header.replyTo).toBe("assign1");
  ...
  it("non-matchmaker gets pool.role_required", () => {
    ...
    expect((c1.sent[0].payload as any)?.error?.code).toBe("pool.role_required");
  });
  ...
  it("rejects pool.claim in delegated mode", () => {
    ...
    expect((c1.sent[0].payload as any)?.error?.code).toBe("pool.mode_mismatch");
  });
});
...
describe("error paths", () => {
  ...
  it("pool.leave on nonexistent pool returns pool.not_found", () => {
    hub.handler.dispatch(c1, {
      header: { id: "l1", resource: "pool", method: "leave", kind: "request" },
      payload: { pool: "nonexistent" },
    });
  ...
  it("pool.leave when not a member returns pool.not_member", () => {
    ...
    expect((c1.sent[0].payload as any)?.error?.code).toBe("pool.not_member");
  });
  ...
  it("pool.claim on nonexistent target returns pool.target_not_found", () => {
    ...
    expect((c1.sent[0].payload as any)?.error?.code).toBe("pool.target_not_found");
  });
  ...
  it("pool.assign with wrong group size returns pool.invalid_group", () => {
    ...
    expect((c1.sent[0].payload as any)?.error?.code).toBe("pool.invalid_group");
  });
