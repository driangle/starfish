...
- [x] Set up Swift package (`Package.swift`) with project structure, dependencies (e.g., `swift-nio-websocket-client` or `URLSessionWebSocketTask`), and build targets
- [x] Implement WebSocket transport layer with connection lifecycle, reconnection with exponential backoff, and protocol handshake (`client.hello` / `server.welcome`)
- [x] Implement session management: join/leave sessions, track connected clients and peers via `AsyncStream` or Combine publishers
- [x] Implement pub/sub topics: subscribe/unsubscribe, publish messages, per-topic event streams
- [x] Implement direct messaging: send to specific clients, broadcast to peers
- [x] Implement presence: set and observe presence data for connected clients
- [x] Implement shared data: save/get with scoped operations (self/session), support data operations (replace, merge, set add/remove, list add/remove, counter add, delete), observable change streams per key
- [x] Implement clock/timing: server time synchronization, scheduled callbacks at specific server times
- [x] Implement event system: global event stream with filtering by type, topic, and sender
...
- [x] Write API documentation with DocC and usage examples in README
