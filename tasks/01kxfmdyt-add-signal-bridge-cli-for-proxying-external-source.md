---
title: "Add signal bridge CLI for proxying external sources (MIDI, OSC) into starfish topics"
id: "01kxfmdyt"
status: pending
priority: medium
type: feature
tags: ["bridge", "cli", "midi", "osc"]
created_at: "2026-07-14"
---

# Add signal bridge CLI for proxying external sources (MIDI, OSC) into starfish topics

## Objective

Create a `@starfish/bridge` CLI package that receives external signals (MIDI, OSC) and publishes them to starfish topics. The bridge acts as a protocol translator — it connects to a starfish session on one side and listens to an external source on the other, forwarding incoming messages as topic publishes.
