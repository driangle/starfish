...
- [ ] `pool-auto: three clients entering with groupSize 3 are matched together` — group
      matching only pairs clients; `groupSize > 2` is not honored.
- [ ] `pool-claim: pool.claim in auto mode returns pool.mode_mismatch` — Go returns
      `pool.not_found` instead of `pool.mode_mismatch` when a client claims in a pool
      whose mode does not permit claiming.
- [ ] `pool resume: reconnected client can still be matched` — a client that reconnects
...
- [ ] `pool-delegated: assigning non-existent member returns pool.target_not_found` —
      passed in most runs, failed once; confirm whether this is a race/flake.

## Tasks

- [ ] Implement `groupSize` > 2 matching in the Go server pool matchmaker
- [ ] Return `pool.mode_mismatch` when claiming in a non-claim-mode pool
- [ ] Restore pool matchmaking eligibility after a client resumes
- [ ] Investigate the intermittent `pool.target_not_found` failure
- [ ] Verify `make test-golang` passes with zero failures
