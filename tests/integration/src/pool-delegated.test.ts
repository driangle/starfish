...
describe("pool delegated mode", () => {
  ...
  it("regular member receives pool.entered without member list", async () => {
    ...
    const entered = await member.waitForReply(enter.id);
  ...
  it("matchmaker receives pool.entered", async () => {
    ...
    await mm.send(enter);
    const entered = await mm.waitForReply(enter.id);
  ...
  it("pool.assign matches members and returns pool.assigned", async () => {
    ...
    const assigned = await mm.waitForReply(assign.id);
    expect(assigned.type).toBe("pool.assigned");
    expect(assigned.payload.matched).toBeInstanceOf(Array);
    expect(assigned.payload.matched[0].session).toBeTruthy();
    expect(assigned.payload.matched[0].group).toContain(a.clientId);
    expect(assigned.payload.matched[0].group).toContain(b.clientId);
  ...
  it("assigning non-existent member returns pool.target_not_found", async () => {
    ...
    const reply = await mm.waitForReply(assign.id);
  ...
  it("non-matchmaker sending pool.assign returns pool.role_required", async () => {
    ...
    const reply = await a.waitForReply(assign.id);
    expect(reply.error?.code).toBe("pool.role_required");
  });
  ...
  it("assigning a group with wrong size returns pool.invalid_group", async () => {
    ...
    const reply = await mm.waitForReply(assign.id);
    expect(reply.error?.code).toBe("pool.invalid_group");
  });
});
