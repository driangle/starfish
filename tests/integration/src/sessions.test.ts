...
describe("sessions", () => {
  ...
  it("join with create: true creates session and returns session.joined", async () => {
    ...
    expect(joined.type).toBe("session.joined");
    ...
    expect(joined.payload.clients.length).toBe(1);
    expect(joined.payload.clients[0].id).toBe(client.clientId);
  });
  ...
  it("second client joining triggers client.connected on first", async () => {
    ...
    // Client 1 should receive client.connected about client 2
    const connected = await client1.waitForType("client.connected");
    expect(connected.session).toBe(session);
    expect(connected.payload.client).toBeDefined();
    expect(connected.payload.client.id).toBe(client2.clientId);
  });
  ...
  it("session.joined contains full client list for second joiner", async () => {
    ...
    expect(joined.payload.clients.length).toBe(2);
    const clientIds = joined.payload.clients.map((c: any) => c.id);
  ...
  it("session.leave triggers client.disconnected with reason 'left'", async () => {
    ...
    // Drain the client.connected event from client1
    ...
    // Client 1 should see client.disconnected
    const disconnected = await client1.waitForType("client.disconnected");
  ...
  it("session is ephemeral — destroyed when last client leaves", async () => {
    ...
    const response = await client2.waitForReply(frame.id);
