...
describe("topics", () => {
  ...
  it("subscribe returns topic.subscribed", async () => {
    ...
    const subscribed = await client.waitForReply(sub.id);

    expect(subscribed.type).toBe("topic.subscribed");
    expect(subscribed.session).toBe(session);
    expect(subscribed.topic).toBe("lights");
  });
  ...
  it("publisher receives own message when also subscribed", async () => {
    ...
    await publisher.waitForReply(sub.id);

    // Other also subscribes
    const sub2 = subscribeFrame(session, "lights");
    await other.send(sub2);
    await other.waitForReply(sub2.id);

    // Drain any topic.peers / client.connected events
    ...
    expect(otherMsg.topic).toBe("lights");
    ...
    expect(selfMsg.topic).toBe("lights");
  ...
  it("unsubscribe stops delivery", async () => {
    ...
    await subscriber.waitForReply(unsub.id);
  ...
  it("topic.peers sent after subscription", async () => {
    ...
    await client1.waitForReply(sub1.id);
